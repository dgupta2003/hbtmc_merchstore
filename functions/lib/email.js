...
function resolveRecipient(order) {
    var _a, _b;
    if ((order === null || order === void 0 ? void 0 : order.user_id) === 'guest') {
        return {
            email: ((_a = order === null || order === void 0 ? void 0 : order.guestDetail) === null || _a === void 0 ? void 0 : _a.email) || '',
            name: ((_b = order === null || order === void 0 ? void 0 : order.guestDetail) === null || _b === void 0 ? void 0 : _b.name) || 'Customer',
        };
    }
    return {
        email: (order === null || order === void 0 ? void 0 : order.user_email) || '',
        name: (order === null || order === void 0 ? void 0 : order.user_name) || (order === null || order === void 0 ? void 0 : order.user_roll_number) || 'Student',
    };
}
...
function renderCustomizations(item) {
    ...
    if (item.customizationTexts && typeof item.customizationTexts === 'object') {
        for (const [label, val] of Object.entries(item.customizationTexts)) {
            if (val)
                parts.push(`${escapeHtml(label)}: ${escapeHtml(val)}`);
        }
    }
...
function renderItemsTable(order) {
    ...
    const rows = items
        .map((item) => {
        const sizeLabel = item.size && item.size !== 'N/A' ? ` (Size: ${escapeHtml(item.size)})` : '';
...
async function sendOrderConfirmation(order) {
    try {
        ...
        const inner = `
            ...
            <p style="font-size:13px;color:#6b7280;">Order ID: <strong>${escapeHtml((order === null || order === void 0 ? void 0 : order.id) || '')}</strong></p>
...
exports.sendPickupNotification = sendPickupNotification;
/**
 ...
 */
async function sendProductAnnouncement(product, recipients) {
    try {
        if (!configured())
            return false;
        ...
        const imageBlock = (product === null || product === void 0 ? void 0 : product.image_url)
        ...
        const inner = `
            ...
            ${imageBlock}
            <h3 style="margin:8px 0 4px;color:#111827;">${escapeHtml(product === null || product === void 0 ? void 0 : product.name)}</h3>
            <p style="font-size:20px;font-weight:700;color:#1a2e5a;margin:0 0 16px;">₹${escapeHtml(product === null || product === void 0 ? void 0 : product.price_inr)}</p>
        ...
        const subject = `New Drop: ${String((product === null || product === void 0 ? void 0 : product.name) || 'New Product').replace(/[\r\n]/g, '')} — HBTMC Gymkhana Emporium`;
...
exports.sendProductAnnouncement = sendProductAnnouncement;
